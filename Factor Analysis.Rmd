...
*We initially tried a different and less judiciously chosen set of predictors, which gave a KMO value of about 0.65. This number, while also probably acceptable for factora analysis, is on the low side. As such, we decided to choose a better sample. The indicators we chose previously were columns 4, 6, 9, 16, 17, 18, 19, 29, 30, 31.*  
...
```{r}
fact_pca <- fa(data, nfactors=4, SMC=FALSE, fm="pa", rotation = 'NONE')
fact_pca
```
...
fact_paf <- fa(data, nfactors=4, fm="pa", rotation = 'NONE')
fact_paf
```
...
```
*All three methods -- iterative PCA, maximum likelihood, and principal axis factoring -- provide strikingly similar results. We will test both iterative PCA and PAF with our rotations below. The loading plots look good for our analysis. As we can see, there is clearly a lot of variation explained by our two axis factors (the first two factors), but there also seems to be at least one more factor.The next step should provide some clarity.*
