...
print(output1$KMO)
summary(output1)
#The selected indicators give us a Kaiser-Meyer-Olkin output of 0.74211, which is acceptable. We can proceed with Factor Analysis.
...
#components 1-35 greater than one, we will use a different metric to determine number of latent factors, because this seems to be too many, and not necessarily that helpful. 

#scree plot
plot(fit, type="lines", main="Scree Plot of FIFA Data",col="red",lwd=2,pch=19,cex=1.2)
#elbow at 3rd component. We will aim for 3 latent factors in our factor analysis. 
```
...
For the sake of comparison, we have also included Factor Analysis using PAF with a varimax rotation.
```{r}
#this uses the fa() function in the psych package.  Note that this fails with only 2 factors
fact_paf <- factanal(data, factors=3, rotation="varimax", fm="pa", lower = 0.02)
fact_paf$PVAL
```
...
PDmid <- PD2[ which(PD2$Position == "CM"), ] #| PD2$Position == "CDM" | PD2$Position == "CAM" | PD2$Position == "LAM" | PD2$Position == "RAM" | PD2$Position == "LCM" | PD2$Position == "RCM" | PD2$Position == "LM" | PD2$Position == "RM"), ]
...
PDoff <- PD2[ which(PD2$Position == "ST"), ] #| PD2$Position == "RS" | PD2$Position == "LS" | PD2$Position == "LW" | PD2$Position == "RW" | PD2$Position == "CF" | PD2$Position == "LF" | PD2$Position == "RF"), ]
...
defp2 <- fviz_cluster(defk2, geom = "point", data = SPDoff) + 
  ggtitle("k = 2")
defp3 <- fviz_cluster(defk3, geom = "point",  data = SPDoff) + 
  ggtitle("k = 3")
defp4 <- fviz_cluster(defk4, geom = "point",  data = SPDoff) + 
  ggtitle("k = 4")
defp5 <- fviz_cluster(defk5, geom = "point",  data = SPDoff) + 
  ggtitle("k = 5")

marrangeGrob(grobs=list(defp2, defp3, defp4, defp5), ncol=2, nrow = 2)
```
#### run k-means with center midfielders


```{r}
fviz_nbclust(SPDmid, kmeans, method = "silhouette") +
labs(title= "Optimal Cluster Number for Center Midfielders - Silhouette Method") 
fviz_nbclust(SPDmid, kmeans, method = "wss") +
labs(title= "Optimal Cluster Number for Center Midfielders - WSS Method") +
geom_vline(xintercept = 3 , linetype = 2)
...
midp2 <- fviz_cluster(midk2, geom = "point", data = SPDoff) + 
  ggtitle("k = 2")
midp3 <- fviz_cluster(midk3, geom = "point",  data = SPDoff) + 
  ggtitle("k = 3")
midp4 <- fviz_cluster(midk4, geom = "point",  data = SPDoff) + 
  ggtitle("k = 4")
midp5 <- fviz_cluster(midk5, geom = "point",  data = SPDoff) + 
  ggtitle("k = 5")
