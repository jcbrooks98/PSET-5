...
Of the two factor analyses methods used, the maximum likelihood extraction method with varimax rotation produced a lower RMSR value and also had the smaller proportion of residuals greater than .05. As such, the following analyses will be based off of the maximum likelihood method (the principal axis factoring method is meant only to be used for comparison's sake, and no conclusions will be made from this method). 
